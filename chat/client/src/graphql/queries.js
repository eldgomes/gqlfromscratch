...
export async function onMessageAdded(handleMessage) {
  const observable = client.subscribe({query: messageAddedSubscription}); //returns an observable
  return observable.subscribe(result => handleMessage(result.data.messageAdded)); //return is for cleanup
}